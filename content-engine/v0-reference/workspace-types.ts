// V0 Reference — WorkspaceProject types and helpers
// This file is for reference only. Adapt for use in workspace-types.ts
...
export function isArticleType(type: WorkspaceContentType): boolean {
  return type === "itinerary_cluster" || type === "authority" || type === "designer_insight"
}

export function isCompoundType(type: WorkspaceContentType): boolean {
  return type === "destination_page" || type === "property_page"
}

export function getTabsForContentType(type: WorkspaceContentType): string[] {
...
export function getNextStage(stage: string, contentType: string): string | null {
  if (isArticleType(contentType as WorkspaceContentType)) return ARTICLE_ADVANCE[stage] || null
  if (isCompoundType(contentType as WorkspaceContentType)) return PAGE_ADVANCE[stage] || null
